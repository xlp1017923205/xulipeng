...
use app\admin\model\Brand;//因为不是每一个controller用到的model都是admin'
...
class BrandController extends Controller{
 ...
 //判断搜索条件中是否具有logo条件
        $logo = input('logo','');//获取页面传来的值
        if(''!='%$field%'){
            //这里是将用户输入的内容添加到查询中，得要查询后的数据
            $model->where('logo','like','%'.$logo.'%');
            //将查询条件放回到分页中，防止点击下一页时，页面内容回到第一页
            $field['logo']=$logo;
            //这个操作为了搜索栏的回显
 ...
 //判断搜索条件中是否具有site条件
        $site = input('site','');//获取页面传来的值
        if(''!='%$field%'){
            //这里是将用户输入的内容添加到查询中，得要查询后的数据
            $model->where('site','like','%'.$site.'%');
            //将查询条件放回到分页中，防止点击下一页时，页面内容回到第一页
            $field['site']=$site;
            //这个操作为了搜索栏的回显
...
public  function set($id=null){
    ...
    $request= request();
      ...
      //自动校检
        if(is_null($id)){
            checkPrivRedirect("admin/brand-create");
        }    else{
            checkPrivRedirect("admin/brand-edit");
        }
